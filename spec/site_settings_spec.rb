...
describe "DiscourseStaffAlias plugin site settings" do
  describe "enabled site setting" do
    it "raises the right error when trying to enable without setting the alias username" do
      expect { SiteSetting.set(:discourse_staff_alias_enabled, true) }
        .to raise_error(
  ...
  describe "site setting changed discourse event" do
    it 'should create the staff alias user if it has not been created' do
      expect do
        SiteSetting.set(:discourse_staff_alias_username, 'new_username')
      end.to change { User.count }.by(1)

      user = User.find(SiteSetting.get(:discourse_staff_alias_user_id))
    ...
    it 'should update the username of the staff alias user' do
      expect do
        SiteSetting.set(:discourse_staff_alias_username, 'new_username')
        SiteSetting.set(:discourse_staff_alias_username, 'new_username2')
      end.to change { User.count }.by(1)
    ...
    it 'should not allow an invalid username' do
      ...
      expect do
        SiteSetting.set(:discourse_staff_alias_username, 'a')
      end.to raise_error(Discourse::InvalidParameters, /#{I18n.t("user.username.short", min: 2)}/)
    end

    it 'should not allow a username that has been taken' do
      expect do
        SiteSetting.set(:discourse_staff_alias_username, 'system')
      end.to raise_error(Discourse::InvalidParameters, /#{I18n.t("login.not_available", suggestion: "system1")}/)
    end
  end
end
