...
describe PostsController do
  ...
  describe "#create" do
    ...
    it "advances the draft sequence for the staff user" do
      ...
      expect do
        ...
        expect(response.status).to eq(200)
      end.to change { alias_user.posts.count }.by(1).and change {
              Draft.where(user_id: moderator.id).count
            }.by(-1).and change {
                    DraftSequence.exists?(user_id: moderator.id, draft_key: post_1.topic.draft_key)
                  }.from(false).to(true)
    end
    ...
    it "allows a staff user to post as alias user" do
      ...
      expect(
        DiscourseStaffAlias::UsersPostsLink.exists?(user_id: moderator.id, post_id: post.id),
      ).to eq(true)

      expect(TopicUser.get(post.topic, moderator).notification_level).to eq(
  ...
  describe "#update" do
    ...
    it "advances the draft sequence for the staff user" do
      ...
      expect do
        ...
        expect(response.status).to eq(200)
      end.to change { post_2.revisions.count }.by(1).and change {
              Draft.where(user_id: moderator.id).count
  ...
  describe "#post_type" do
    it "allows users in staff aliased allowed group to update post_type of post made by staff alias user" do
      ...
      expect do
        put "/posts/#{new_post.id}/post_type.json",
            params: {
      ...
      expect(post_revision.post_id).to eq(new_post.id)
      expect(post_revision.user_id).to eq(DiscourseStaffAlias.alias_user.id)
      expect(post_revision.modifications).to eq(
